...
# tf.keras.utils.set_random_seed(2020) # not used
random.seed(2020)
np.random.seed(2020)
...
class LSTM_TrajGAN():
    def __init__(self, latent_dim, keys, vocab_size, max_length, lat_centroid, lon_centroid, scale_factor, private=False, noise_multiplier=0.01, max_grad=50, lr=0.001):
        ...
        if not private:
            ...
            self.optimizer = tensorflow_privacy.DPKerasSGDOptimizer(self.max_grad, self.noise_multiplier, 1, lr)
    ...
    def train(self, epochs=200, batch_size=256, sample_interval=10, dataset ='foursquare'):
        
        # Training data
        dataset_path = 'final_train'
        x_train = np.load(f'data/{dataset}/{dataset_path}.npy',allow_pickle=True)
